...
public:
    static Complexity correctComplexity(const juce::String& originalPrompt, Complexity predictedComplexity, WorkType type) {
        ...
        if (predictedComplexity == Complexity::Low && baseline == Complexity::High) {
            return Complexity::High; // Correcting automatically
        }
